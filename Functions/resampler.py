...
class Resampler():
    ...
    def __init__(self,method = 'random over', alpha = None):
        self.method = method
        self.alpha = alpha
    ...
    def define_resampler(self):
        ...
        sampling_strategy = 'auto' if self.alpha is None else self.alpha
        self.under = False
        
        if 'RANDOM' in self.method.upper() :
            if 'UNDER' in self.method.upper() :
                self.resampler = RandomUnderSampler(sampling_strategy = sampling_strategy)
                self.under = True
            else :
                self.resampler = RandomOverSampler(sampling_strategy = sampling_strategy)
        elif 'SMOTE' in self.method.upper() :
            if 'K' in self.method.upper() :
                self.resampler = KMeansSMOTE(sampling_strategy = sampling_strategy)
            elif 'B' in self.method.upper() :
                self.resampler = BorderlineSMOTE(sampling_strategy = sampling_strategy)
            elif 'V' in self.method.upper() :
                self.resampler = SVMSMOTE(sampling_strategy = sampling_strategy)
            elif 'N' in self.method.upper() :
                self.resampler = SMOTEN(sampling_strategy = sampling_strategy)
            else :
                self.resampler = SMOTE(sampling_strategy = sampling_strategy)
        elif 'ADASYN' in self.method.upper() :
            self.resampler = ADASYN(sampling_strategy = sampling_strategy)
